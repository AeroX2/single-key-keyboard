...
void HIDSerialDevice::poll()
{
    usbPoll();
}

uchar HIDSerialDevice::available()
{
    return received;
}

uchar HIDSerialDevice::read(uchar *buffer)
{
...
// write one character
size_t HIDSerialDevice::write(uint8_t data)
{
...
size_t HIDSerialDevice::write8(const uint8_t *buffer, size_t size)
{
  unsigned char i;
  while(!usbInterruptIsReady()) {
    usbPoll();
...
// write a string
size_t HIDSerialDevice::write(const uint8_t *buffer, size_t size)
{
  size_t count = 0;
  unsigned char i;
